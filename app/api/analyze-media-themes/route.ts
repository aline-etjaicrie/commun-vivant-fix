...
À partir de ces éléments, tu proposes une liste de thèmes dominants.

Tu produis uniquement une liste de 8 à 12 thèmes courts.
Pas de phrases.
Pas de narration.
Pas d'interprétation intime.`;

const USER_PROMPT = `Voici les images d'une personne destinées à un hommage.
Analyse-les et propose les thèmes dominants.`;

function normalizeThemes(raw: string): string[] {
  return raw
    .split(/\r?\n|,/g)
...
export async function POST(request: NextRequest) {
  try {
    const { memory_id, image_urls = [] } = await request.json();
    ...
    if (!Array.isArray(image_urls) || image_urls.length === 0) {
      return NextResponse.json({ error: 'image_urls manquantes' }, { status: 400 });
    }
    ...
    const limitedImages = image_urls
    ...
    if (!response.ok) {
      ...
      console.error('THEMES analyze error:', errorText);
      return NextResponse.json({ error: 'Erreur analyse visuelle' }, { status: 500 });
    }

    const data = await response.json();
    const text = data.choices?.[0]?.message?.content?.trim() || '';
    const proposedThemes = normalizeThemes(text);

    if (proposedThemes.length === 0) {
      return NextResponse.json({ error: 'Analyse vide' }, { status: 500 });
    }

    return NextResponse.json({ memory_id, proposedThemes });
  } catch (error) {
    console.error('THEMES analyze server error:', error);
